...
wt <- 1 / abs(coxph(Surv(breast_ca$SRV_TIME_MON, breast_ca$delta) ~ X)$coefficient)
alasso.fit <- ncvsurv(X, Surv(breast_ca$SRV_TIME_MON, breast_ca$delta), penalty = "lasso", 
                      nlambda = 25, penalty.factor = wt)
